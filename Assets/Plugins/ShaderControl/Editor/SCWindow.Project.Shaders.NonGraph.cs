    ...
    public partial class SCWindow : EditorWindow {
        ...
        void ScanShaderNonGraph(SCShader shader) {
            ...
            for (int k = 0; k < shaderLines.Length; k++) {
                ...
                PragmaType pragmaType = PragmaTypeUtils.GetPragmaType(line, out j, out int offset);
                if (j >= 0) {
                    if (pragmaControl != 2) {
        ...
        void UpdateShaderNonGraph(SCShader shader) {
            ...
            for (int k = 0; k < shaderLines.Length; k++) {
                ...
                if (!blockComment) {
                    ...
                    pragmaType = PragmaTypeUtils.GetPragmaType(shaderLines[k], out j, out _);
        ...
        void ConvertToLocalNonGraph(SCKeyword keyword, SCShader shader) {
            ...
            for (int k = 0; k < lines.Length; k++) {
                // Just convert to local shader_features for now since multi_compile global keywords can be enabled using the Shader global API
                ...
                if (lines[k].IndexOf(SCKeywordLine.PRAGMA_FEATURE_GLOBAL_FRAGMENT, StringComparison.InvariantCultureIgnoreCase) >= 0 && lines[k].IndexOf(keyword.name, StringComparison.InvariantCultureIgnoreCase) >= 0) {
                    lines[k] = lines[k].Replace(SCKeywordLine.PRAGMA_FEATURE_GLOBAL_FRAGMENT, SCKeywordLine.PRAGMA_FEATURE_LOCAL_FRAGMENT);
                    lines[k] = lines[k].Replace(SCKeywordLine.PRAGMA_FEATURE_GLOBAL_FRAGMENT.ToUpper(), SCKeywordLine.PRAGMA_FEATURE_LOCAL_FRAGMENT);
                    changed = true;
                }
            }
            if (changed) {
